...
class akahu2firefly():
    ...
    def main(self):
        ...
        self.handled_akahu_transaction_types = ['TRANSFER', 'DEBIT', 'DIRECT DEBIT', 'EFTPOS', 'TAX', 'INTEREST', 'FEE', 'CREDIT']
        ...
        configuration = firefly_iii_client.Configuration(
            ...
            access_token = os.getenv('FIREFLY_PAT')
        )
    ...
    def update_starting_balance_for_firefly_accounts(self):
        ...
        for akahu_account_id, firefly_account_id in self.akahu2firefly.items():
            ...
            if(result):
                ...
                account: AccountSingle = accounts_api_client.get_account(firefly_account_id)
                attributes = account.data["attributes"]
                update = AccountUpdate(attributes["name"] , opening_balance = str(starting_balance), opening_balance_date = dt.date())
                result: AccountSingle = accounts_api_client.update_account(firefly_account_id, update)

    def create_mapping_dicts_from_db(self):
        mappings = self.dbcon.execute('SELECT akahu_account_id, firefly_account_id from akahu2firefly_account').fetchall()
        for akahu_account_id, firefly_account_id,  in mappings:
    ...
    def update_transaction_record(self, akahu_transaction):
        ...
        akahu_transaction_created_at = parser.parse(akahu_transaction['date']).timestamp()
    ...
    def update_firefly_transaction_record(self, akahu_transaction, firefly_transaction_id):
        # TODO
        print(f"update firefly transaction {firefly_transaction_id} from akahu_transaction {akahu_transaction['_id']}")
    ...
    def create_kiwibank_transfer(self, akahu_transaction):
        ...
        matcher = re.compile('(?P<control>TRANSFER FROM|TRANSFER TO).+ - (?P<account>[0-9][0-9])')
        match = matcher.match(description)
        if len(match.groups()) != 2:
            print(f"unable to match transfer description as expected {akahu_transaction['id']}")
            return
    ...
    def create_transfer(self, akahu_transaction, target_firefly_account_id):
        ...
        akahu_transaction_updated_at = parser.parse(akahu_transaction['updated_at']).timestamp()
        ...
        try:
            ...
            self.dbcon.execute('''INSERT OR REPLACE INTO akahu2firefly_transaction VALUES (?, ?, ?)''', [akahu_transaction_id, data.id, akahu_transaction_updated_at])
            self.dbcon.commit()
    ...
    def get_category_and_tags(self, akahu_transaction):
        category = None
        tags = []
        if 'category' in akahu_transaction and 'components' in akahu_transaction['category']:
    ...
    def create_withdrawal(self, akahu_transaction):
        ...
        firefly_account = self.firefly_accounts_by_account_id[firefly_account_id]
        akahu_transaction_id = akahu_transaction["_id"]
        akahu_transaction_updated_at = parser.parse(akahu_transaction['updated_at']).timestamp()
        transaction_api_client = transactions_api.TransactionsApi(self.api_client)
        transaction_store = TransactionStore(
    ...
    def create_account_in_firefly(self,account, akahu_account_id):
        ...
        self.dbcon.execute('''INSERT OR REPLACE INTO akahu2firefly_account VALUES (?, ?)''', [akahu_account_id, data.id])
        self.dbcon.commit()
    ...
    def hydrate_firefly_accounts(self):
        ...
        for firefly_account in firefly_accounts.data:
            ...
            if attributes['account_number']:
                number = re.match(r"\d\d-\d\d\d\d-\d\d\d\d\d\d\d-(\d\d)", attributes['account_number'])
